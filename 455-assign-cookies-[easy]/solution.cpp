// Assign Cookies
// started 4/10/17 6:15PM - 7:54PM
// TODO: when writing code in C or C++, always check if ';' are in the right place
#include <time.h>
#include <stdlib.h>
#include <stdio.h>
...
public:
    int findContentChildren(vector<int>& g, vector<int>& s) {
        ...
        quick_sort(g);		//O(mlog(m))
        quick_sort(s);		//O(nlog(n))

  		vector<int>::iterator it_g = g.begin();
  		vector<int>::iterator it_s = s.begin();
  	    
  	    while(it_g != g.end() && it_s != s.end()){		//O(min(m, n))
  	        if(*it_g > *it_s)
  	            ++it_s;
  	         else{
  	            ++it_g;
...
private:
	...
	void quick_sort_helper(vector<int>& nums, int start, int end){
		...
		if(start < (index - 1)) //<<-- another ; mistake!! [was "if(start < (index - 1));"]
			quick_sort_helper(nums, start, index - 1);
		if(index < end)
	...
	int partition(vector<int>& nums, int start, int end){
		...
		int pivot = nums[(rand() % length) + start];
